...
在 JavaScript 中，函数可以被传递，用作对象。这篇笔记用于记录函数之间的转发以及如何装饰函数的学习过程。

## 简单示例——透明缓存

假设我们有一个 CPU 重负载的函数 `slow(x)`，但它的结果是稳定的。换句话说，对于相同的 `x`，它总是返回相同的结果。

如果经常调用该函数，我们可能希望将结果缓存下来，以避免在重新计算上花费额外的时间。
...
其思想是，我们可以为任何函数调用 `cachingDecorator`，它将返回缓存包装器。因为我们有很多函数可以使用这样的特性，而我们需要做的就是将 `cachingDecorator` 应用于它们。
...
1. 作者首先通过透明缓存的例子引出装饰器；
2. 第一版透明缓存装饰器的代码还存在上下文问题，所以又引出了 func.call，使用 func.call 可以解决该问题；
3. 在第二步的基础上进一步完善，解决透明缓存器只能传递单参数的问题；
4. 随后又介绍了与 func.call 功能类似的 func.apply；
5. 再进一步完善 hash 函数，因为不确定用户会传多少个参数，在 hash 函数内部写死 arg[0] + arg[1]，显然不太合理，所以又引出了 method borrowing  的概念，并运用它在 hash  函数中；
6. 最后进行了总结。
...
20240421 22:29 装饰器的参数是函数，返回也是函数。有一个地方不太理解：为什么声明 cache 变量是在返回函数之前？

## 参考

[装饰器模式和转发，call/apply (javascript.info)](https://zh.javascript.info/call-apply-decorators#funcapply)